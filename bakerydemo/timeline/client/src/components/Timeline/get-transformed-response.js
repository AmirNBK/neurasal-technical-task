...
const getTransformedItems = ({ items = [] } = {}) => items.map(({
  meta: { first_published_at, type, ...meta }, ...item
}) => ({
  ...item,
  ...meta,
  group: type,
...
const getDefaultTimes = (items) => items.reduce(({ start = null, end = null },
  { start_time, end_time }) => {
